...
public class SocioFederadoDAOImpl implements SocioFederadoDAO {
    ...
    public ArrayList<SocioFederadoModel> obtenerTodosSocioFederado() throws SQLException {
        ...
        ArrayList<SocioFederadoModel> socios = new ArrayList<>(); // Se crea una lista para almacenar los socios
                                                                  // federados encontrados
        try {
            pst = con.prepareStatement("SELECT * FROM socioFederado"); // Se prepara una consulta SQL para seleccionar
                                                                       // todos los socios federados
            ResultSet respuestaBD = pst.executeQuery(); // Se ejecuta la consulta y se almacena el resultado en un
                                                        // ResultSet
            while (respuestaBD.next()) { // Mientras haya resultados en el ResultSet...
                String codigoFederacion = respuestaBD.getString("codigoFederacion"); // Se obtiene el código de
                                                                                     // federación del resultado
                FederacionDAO federacionDAO = new FederacionDAOImpl(); // Se crea una instancia de FederacionDAOImpl
                FederacionModel federacion = federacionDAO.obtenerPorCodigo(codigoFederacion); // Se obtiene la
                                                                                               // federación
                                                                                               // correspondiente al
                                                                                               // código obtenido
                SocioFederadoModel socio = new SocioFederadoModel( // Se crea un objeto SocioFederadoModel con los datos
                                                                   // del resultado
                        ...
                        federacion // Se asigna el objeto FederacionModel obtenido al atributo de federación del
                                   // objeto SocioFederadoModel
                );
            ...
            throw new SQLException("Fallo en la consulta SQL al obtener todos los socios federados."); // Se lanza una
                                                                                                       // excepción
                                                                                                       // indicando el
                                                                                                       // error
        } finally {
        ...
        try {
            ...
            try (ResultSet respuestaBD = pst.executeQuery()) {
                if (respuestaBD.next()) {
                    ...
                    FederacionModel federacion = federacionDAO
                            .obtenerPorCodigo(respuestaBD.getString("codigoFederacion"));
    ...
    public void crearSocioFederado(SocioFederadoModel socio) throws SQLException {
        ...
        try {
            pst = con.prepareStatement(
                    "INSERT INTO socioFederado (numeroSocio, nombre, NIF, codigoFederacion) VALUES (?, ?, ?, ?)");
