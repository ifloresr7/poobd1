...
public class InscripcionDAOImpl implements InscripcionDAO {
    ...
    public ArrayList<InscripcionModel> obtenerTodasLasInscripciones() throws SQLException {
        ArrayList<InscripcionModel> inscripciones = new ArrayList<>(); // Se crea una lista para almacenar las
                                                                       // inscripciones
        // Se inicia un bloque try-with-resources para manejar la conexión y los
        // recursos JDBC
        ...
        try {
            ...
            while (respuestaBD.next()) {
                // Se crean objetos InscripcionModel utilizando los valores de las columnas del
    ...
    public ArrayList<InscripcionModel> obtenerTodasPorNumeroSocio(int numeroSocio) throws SQLException {
        ...
        try {
            ...
            throw new SQLException(
                    "Fallo en la consulta SQL al obtener todas las inscripciones para el socio con número: "
                            + numeroSocio);
        } finally {
            if (pst != null) {
                pst.close(); // Se cierra el statement.
    ...
    public void crearInscripcion(InscripcionModel inscripcion) throws SQLException {
        ...
        try {
            pst = con.prepareStatement(
                    "INSERT INTO inscripcion (numeroInscripcion, numeroSocio, numeroExcursion, fechaInscripcion) VALUES (?, ?, ?, ?)");
            ...
            pst.setDate(4, new java.sql.Date(inscripcion.getFechaInscripcion().getTime())); // Convertir la fecha de
                                                                                            // Java a SQL
    ...
    public void eliminarInscripcion(int numeroInscripcion) throws SQLException {
        ...
        try {
            ...
            throw new SQLException(
                    "Fallo en la consulta SQL al eliminar la inscripción con número: " + numeroInscripcion);
        } finally {
            if (pst != null) {
                pst.close(); // Se cierra el statement.
            }
